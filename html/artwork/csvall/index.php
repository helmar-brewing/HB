...
if(isset($user)){
   ...
   /* do this code if user is logged in */
      ...
      $R_cards = $db_main->query("SELECT * FROM cardList");
               ...
               $result = $db_main->query("SELECT cardList.*, series_info.series_name FROM cardList LEFT JOIN series_info
		ON cardList.series = series_info.series_tag");
               if($result != FALSE){
                 $result->data_seek(0);
                 while($object = $result->fetch_object()){
                   // your csvline needs to be a simple array
                   $csvline = array($object->series_name,$object->cardnum,$object->player,$object->description,$object->team,$object->lastsold,$object->maxSold);
                   fputcsv($fp, $csvline);
                 }
               }
