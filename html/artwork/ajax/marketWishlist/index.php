...
//	}
	...
	// get current qty
	$quantity = db1($db_main, "SELECT marketWishlist FROM userCardChecklist WHERE userid='".$user->id."' AND series='$series' and cardnum='".$_GET['cardnum']."' LIMIT 1");
	$quantity2 = db1($db_main, "SELECT quantity FROM marketWishlist WHERE userid='".$user->id."' AND series='$series' and cardnum='".$_GET['cardnum']."' LIMIT 1");
	...
	if($quantity === '0'){
		...
		$db_main->query("UPDATE userCardChecklist SET marketWishlist=1 WHERE userid='".$user->id."' AND series='".$series."' and cardnum='".$_GET['cardnum']."' LIMIT 1");
		if($quantity2==='none'){
			$db_main->query("INSERT INTO marketWishlist (userid, series, cardnum, quantity, lastBumpDate, endDate, expired) VALUES ('".$user->id."', '$series', '".$_GET['cardnum']."', 1, $now, $expire, 'N')");
		}else{
		...
		$db_main->query("UPDATE userCardChecklist SET marketWishlist=0 WHERE userid='".$user->id."' AND series='".$series."' and cardnum='".$_GET['cardnum']."' LIMIT 1");
		if($quantity2==='none'){
			// do nothing, this elseif is removing a value and it doesn't exist so no need to update marketplace table
		}else{
			$db_main->query("UPDATE marketWishlist SET quantity=0, endDate=$now, expired='Y' WHERE userid='".$user->id."' AND series='".$series."' and cardnum='".$_GET['cardnum']."' LIMIT 1");
		}
		$msg = 'You have removed card '.$_GET['cardnum'].' from your wishlist';
	}elseif($quantity === 'none'){
		$db_main->query("INSERT INTO userCardChecklist (userid, series, cardnum, quantity, marketWishlist) VALUES ('".$user->id."', '$series', '".$_GET['cardnum']."', 0, 1)");
		if($quantity2==='none'){
			$db_main->query("INSERT INTO marketWishlist (userid, series, cardnum, quantity, lastBumpDate, endDate, expired) VALUES ('".$user->id."', '$series', '".$_GET['cardnum']."', 1, $now, $expire, 'N')");
		}else{
			$db_main->query("UPDATE marketWishlist SET quantity=1, endDate=$expire, expired='N' WHERE userid='".$user->id."' AND series='".$series."' and cardnum='".$_GET['cardnum']."' LIMIT 1");
	...
	$qty = db1($db_main, "SELECT marketWishlist FROM userCardChecklist WHERE userid='".$user->id."' AND series='".$series."' AND cardnum='".$_GET['cardnum']."' LIMIT 1");
