...
function cancel(){
	...
	$html = '
		...
		<button onclick="subUpdate2(\''.$action.'\')">Disable Auto-Renew</button>
	';
	return $html;
}
function renew($to){
	...
	if($user->subscription['cancel_at_period_end'] === true){
		...
		$h .= '<button onclick="subUpdate2(\''.$action.'\')">Enable Auto-Renew</button>';
	}else{
		...
		if($meta['downgrade'] === 'yes'){
			...
			$h .= '<button onclick="subUpdate2(\''.$action.'\')">Change Subscription</button>';
		}else{
