	...
	if($user->subscription['status'] === 'none'){
		...
		switch($action){
			...
			case 'subscribe' : {
				...
				$html .= '<button onclick="subUpdate2(\'subscribe\')">Subscribe</button>';
				break;
			}
			default : {
		...
		switch($action){
			case 'cancel' : {
				...
				if($user->subscription['cancel_at_period_end'] === TRUE){
					...
					$html .= '<button onclick="subUpdate2(\'cancel\')">Disable Auto-Renew</button>';
				}
				break;
			}
			case 'subscribe' : {
				...
				if($user->subscription['cancel_at_period_end'] === TRUE){
					...
					$html .= '<button onclick="subUpdate2(\'subscribe\')">Enable Auto-Renew</button>';
				}else{
